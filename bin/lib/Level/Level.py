...
class Level:
    ...
    def get_neighbors(self, position={"X": 0, "Y": 0}):
        ...
        for y in range(3):
            ...
            for x in range(3):
                ...
                currentNeighbor["X"] = position[
                    "X"].copy()  # currentNeighbor wird nach durchlauf manipuliert sein, setze current Neighbor zurück auf position(erste NachbarPos ecke oben links)
                currentNeighbor["X"] += x
                ...
                if (currentNeighbor["X"] < 0):
                    ...
                    DEBUG("Level.get_neighbors(position{}) Rand erkannt, wähle" + currentNeighbor["X"] + "als Nachbarn",
                          4)
    ...
    def parse_lvl_file(self, filePath = ""):
        ...
        if (os.path.isfile(filePath)):  # datei vorhanden?
            ...
            DEBUG("Level.parse_lvl_file: DateiInhalt: ", 2, lvlFileObj)
            lvlFile = []
            for line in lvlFileObj:
                lvlFile.append(line)
            DEBUG("Level.parse_lvl_file: Datei in Stringliste umgewandelt:", 1, lvlFile)
            if (len(lvlFile) > 0):  # inhalt nicht leer?
                lvlData = []
                for x in lvlFile:
                    x = x.replace(" ", "")
                    DEBUG("Level.parse_lvl_file: bearbeite", 3, x)
                    lvlData.append(x)
                DEBUG("Level.parse_lvl_file: Datei getrimmt und Platzhalter gefüllt:", 2, lvlData)
                ...
                for line in lvlData:
                    ...
                    for conditionName in DATA_CONDITIONS_LVL:
                        ...
                        DEBUG("Level.parse_lvl_file: nutze Zur Suche von " + conditionName + " diesen RegEx-Ausdruck: ",
                              3, condition)
                        results = re.search(condition, line)  # hier testen, ob auch lvlFile übergeben werden kann (denn dann ist zeile 32 unnötig)

                        if (results != None):
                            DEBUG("Level.parse_lvl_file: gefundene Zeile(n):\n", 5, results)
                            if conditionName == "grid":
                                DEBUG("Level.parse_lvl_file: grid Zeilen gefunden", 4)
                                for x in results:
                                    x = x.replace("GRID=", "")
                                    ...
                                    if (len(tileTypeCols) > rawLvl["maxWidth"]):
                                        ...
                                        DEBUG("Level.parse_lvl_file: längste Spalte gefunden, Tilemap hat jetzt " +
                                              rawLvl["maxWidth"] + " Spalten", 5)
                                    rawLvl["grid"].add(list(map(int,
                                                                tileTypeCols)))  # wandelt String-liste in int-liste um und fügt sie der tileIDMap zu
                            elif conditionName == "difficulty":
                                DEBUG(
                                    "Level.parse_lvl_file: difficulty Paramter gefunden, wähle: ",
                                    4, results[-1])

                                extractedDifficulties = list(
                                    map(int, results[-1].replace("difficulty=", "")))  # gruselige Zeile =)
                                rawLvl["difficulty"] = extractedDifficulties[-1]

                            elif conditionName == "playerStartPos":
                                DEBUG("Level.parse_lvl_file: playerStartPos Paramter gefunden, wähle: ", 4, results[-1])
                                self.parameters["playerStartPositions"].clear()
                                extractedplayerStartPositions = list(
                                    map(int, results[-1].replace("playerStartPos=", "")))
                                startPosList = extractedplayerStartPositions[-1].split(')(')
                                for x in startPosList:
                                    self.parameters["playerStartPositions"] = list(
                                        map(int, x.replace("(", "").replace(")", "").split(';')))  # gruselig =)
                                    DEBUG("Level.parse_lvl_file: füge (difficulty Paramter gefunden, wähle: ", 5,
                                          self.parameters["playerStartPositions"])
    ...
    def parse_texture_set(self):
        ...
        if (os.path.isdir(textureSetPath) == False):  # wenn texturOrdner nicht exisitiert -> DEFAULT-Textur-Ordner
            DEBUG("Level.parse_texture_set(levelPath): TexturPfad nicht gefunden. falle auf DefaultTexturSet zurück...",
                  1, textureSetPath)
        ...
        DEBUG("Level.parse_texture_set(levelPath): suche nach confFiles mit diesem Ausdruck", 2, textureSetConfRegex)
        foundConfFilePaths = glob.glob(textureSetConfRegex)
        DEBUG("Level.parse_texture_set(levelPath): diese Files wurden gefunden", 3, foundConfFilePaths)
        if(len(foundConfFilePaths) > 0):
            if (foundConfFilePaths[-1] == "none"):  # wenn keine .conf-Datei gefunden -> DEFAULT-Textur-Ordner
                DEBUG("Level.parse_texture_set(levelPath): TexturPfad enthält keine Textur Datei/en(.conf).", 0,
                      textureSetPath)
                textureSetPath = DEFAULT_TEXTURE_SET_PATH
                DEBUG("Level.parse_texture_set(levelPath): Falle auf DefaultTexturSet zurück...", 1, textureSetPath)
            else:
            ...
            for confFilePath in foundConfFilePaths:
                ...
                if (len(confFile) > 0):  # inhalt nicht leer?
                    ...
                    for line in confData:
                        DEBUG(
                            "Level.parse_texture_set: Erzeuge String mit zusammenhängenden DateiInhalt (für re.search()):\n",
                            2, line)
                        confData += line
                    ...
                    if (results[-1] != 'none'):  # durchsuche jeden gefundenen ID Block
                        for x in results:
                            ...
                            for conditionName in DATA_CONDITIONS_TILE:
                                DEBUG("Level.parse_texture_set: nutze zur Suche von " + str(
                                    conditionName) + "den Ausdruck " + str(condition), 3)
                                ...
                                if ((results) > 0):
                                    if (conditionName == "ID"):
                                        ...
                                        if (len(temp) >= 2):
                                            ...
                                            DEBUG(
                                                "Level.parse_texture_set: ausgelesene Liste hat zu wenig elemente, fallback auf default",
                                                1, temp)
                                            temp = DEFAULT_TILE_CONF_PARAMETERS["playerMvManipulation"]
                                        DEBUG("Level.parse_texture_set: nach umwandlung", 5, temp)
                                        rawParameters["playerMvManipulation"] = list(
                                            map(int, temp[:2]))  # wandle die ersten beiden elemente von temp in ints um
                                    ...
                                    rawParameters[
                                        "ID"] = 0  # wenn ein Regex nicht gefunden wird, soll mit ID = 0 das eingelesene Paket ungültig gemacht werden
                                re.purge()
            ...
            if (IS_BUILD_ON_UPDATE):
                ...
                for y in self.tileIDMap:
                    for x in y:
                        ...
                        self.currentTilePos[
                            "X"] += 1  # wird benötigt um entsprechende Position des tiles zu bestimmen
