...
DEFAULT_DIR = os.path.join("bin", "lvl", "000","texture", "tiles") #StandardOrdner, wenn keiner angegeben wird
    ...
    if(list != None):
        try:
            ...
            for item in list:
                counter += 1
                if(top >= counter):
                    convertedList.append("Item " + ("{:0" + str(len(str(len(list)))) +"d}").format(counter) + ": '" + str(item) + "'")
                    #print(convertedList[-1])
                elif(top == counter - 1):
                    convertedList.append("...(" + str(len(list) - top) + " weitere Items ausgeblendet)")
                    #print(convertedList[-1]) 
                elif(top < 0):
                    convertedList.append("Item " + ("{:0" + str(len(str(len(list)))) +"d}").format(counter) + ": '" + str(item) + "'")
...
def regExTester(filePath = DEFAULT_DIR):
